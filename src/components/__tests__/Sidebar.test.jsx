...
describe('Sidebar', () => {
  ...
  it('renderiza os botões de seção (Módulo 3, Monitoramento, Encerramento...)', async () => {
    const user = userEvent.setup()
    renderSidebar()
    await user.click(screen.getByRole('button', { name: 'Abrir menu' }))
    expect(screen.getByRole('button', { name: 'Módulo 3' })).toBeInTheDocument()
  ...
  it('aplica text-blue-600 à seção que corresponde ao pathname atual', async () => {
    const user = userEvent.setup()
    renderSidebar({}, '/monitoramento')
    await user.click(screen.getByRole('button', { name: 'Abrir menu' }))
    const matrizBtn = screen.getByRole('button', { name: 'Monitoramento' })
  ...
  it('chama router.push ao clicar em seção de página diferente', async () => {
    ...
    expect(mockPush).toHaveBeenCalledWith('/monitoramento')
  })
